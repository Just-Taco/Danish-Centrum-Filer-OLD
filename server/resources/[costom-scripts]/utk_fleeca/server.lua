        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Den her Bank Blev Næsten lige røvet. Du skal vente "..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Den bank er igang med at blive røvet.")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, "Du har ik malicious Kort.")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "Der er ikke nok betjente i byen.")
    end
end)
...
AddEventHandler('utk_fh:CheckCop', function()
	...
	if vRP.hasPermission({xPlayer,UTK.PolicePermission}) then
		TriggerClientEvent('utk_fh:IsCop', xPlayer)
	else
		TriggerClientEvent('utk_fh:IsNOTCop', xPlayer)
	end
end)
