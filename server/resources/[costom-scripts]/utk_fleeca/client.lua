                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 and not IsPolice then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Tage Pengene", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 and not IsPolice then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Tage Pengene", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 and not IsPolice then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Tage Pengene", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if IsPolice then
        exports["pNotify:SendNotification"]:SendAlert("inform", "A bank's alarms are triggered!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
            ...
            if IsPolice and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 2.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] For at løse døren op", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] For at løse døren", 0.40)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    exports["pNotify:SendNotification"]:SendAlert("error", "Døren Lukekr om 30 Secunder!")
    Citizen.Wait(30000)
    exports["pNotify:SendNotification"]:SendAlert("error", "Døren Lukker!")
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports["pNotify:SendNotification"]:SendAlert("success", "Hacking complete!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    exports["pNotify:SendNotification"]:SendAlert("error", "Du har 2 minuter intil at kamrane system ser dig.")
...
function SecondDoor(data, key)
    ...
    exports["pNotify:SendNotification"]:SendAlert("success", "Access complete!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
                ...
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    UTKFleeca.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            exports["pNotify:SendNotification"]:SendAlert("error", "You don't have the Secure ID Card.")
                        end
                    end)
                end
            end
