	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				ShowNotification("~c~Du ~b~startet ~c~Med at bryge vape.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("~r~Du holder allerede din vape.")
			end
		else
			ShowNotification("~r~Du kan ikke bruge denne i en bil.")
		end
	else
		ShowNotification("~r~Du kan ikke bruge den når du er død.")
	end
end)
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		ShowNotification("~c~Du ~r~stopped ~c~Er stoppet vape.")
	else
		ShowNotification("~r~Du holder ik en vape.")
	end
end)
