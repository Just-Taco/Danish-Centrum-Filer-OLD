                  ...
                  if business then
                    ...
                    vRP.prompt(player,lang.business.open.prompt_name({30}),"",function(player,name)
                      if string.len(name) >= 2 and string.len(name) <= 30 then
                        ...
                        PerformHttpRequest('https://discord.com/api/webhooks/804281559628644413/GEyKkOWzLnRTvTV6JXCDdWHfkZA5uhJs3Us5jx3aq-Fxr0CS2mIj-tofEXI_n6FW6sJ_', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = "**"..user_id.."** har åbnet **"..name.."** for **"..nuser_id.."**"}), { ['Content-Type'] = 'application/json' })
